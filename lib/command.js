(function() {
  ...
  run = function() {
    var file, input_file, output_file, _i, _len, _ref, _ref1, _ref2, _results;

    if (program.auth) {
      _ref = program.args, config.consumer_key = _ref[0], config.consumer_secret = _ref[1];
      getTokens();
      return;
    }
    _ref1 = program.args, input_file = _ref1[0], output_file = _ref1[1];
    if (program.encode) {
    ...
    if (program.decode) {
      png.decode(input_file, output_file, function(err, file, name) {
        return console.log("Recovered " + file + " -> " + name);
      });
      return;
    }
  ...
  saveConfig = function(callback) {
    return fs.writeFile(configPath, JSON.stringify(config, null, 4), callback);
  };
